    ...
    <div>
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="text-[13px] text-muted-foreground mt-0.5">Hola, <span className="font-medium">{user?.name}</span> — seguí el estado de tus reportes ciudadanos</p>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-border shadow-sm p-5 mb-5">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-[12px] text-muted-foreground mb-1.5 font-medium">Título del problema</label>
              <input type="text" value={title} onChange={e => setTitle(e.target.value)}
                placeholder="Ej: Árbol caído en calle Los Aromos"
                className="w-full px-3 py-2.5 border border-border rounded-lg text-[13px] outline-none focus:border-primary" />
            </div>
            <div>
              <label className="block text-[12px] text-muted-foreground mb-1.5 font-medium">Descripción detallada</label>
      ...
      <div className="grid grid-cols-[1fr_380px] gap-5">
        ...
        <div className="space-y-3">
          ...
          {tickets.map(ticket => (
            <button key={ticket.id} onClick={() => setSelectedTicket(ticket === selectedTicket ? null : ticket)}
        ...
        <div>
          {selectedTicket ? (
            <div className="bg-white rounded-xl border border-border shadow-sm p-5 space-y-5 sticky top-6">
              <div>
